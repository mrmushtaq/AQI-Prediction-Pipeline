...
def load_data() -> pd.DataFrame:
    ...
    try:
        ...
        # Fetch live data ONLY for dashboard display.
        # Do NOT persist it here.
        current = pd.DataFrame([run_live_pipeline(persist=False)])

        current["collection_timestamp"] = pd.to_datetime(
        ...
        if {"city", "collection_timestamp"}.issubset(frame.columns):
            frame = frame.drop_duplicates(
                subset=["city", "collection_timestamp"],
                keep="last",
            )

        frame = (
            frame.sort_values("collection_timestamp")
    ...
    with st.expander("Forecast model details"):
        if metadata:
            ...
            for label, _ in HORIZONS:
                ...
                baseline = info.get("naive_baseline", {})

                model_rows.append(
                    {
                        "Horizon": label.replace("_avg", "").replace("day", "Day "),
                        "Version": info.get("version", "Unknown"),
                        "Train samples": info.get("n_train", "-"),
                        "Test samples": info.get("n_test", "-"),
                        "RMSE": round(float(metrics["rmse"]), 2)
                        if metrics.get("rmse") is not None else "-",
                        "MAE": round(float(metrics["mae"]), 2)
                        if metrics.get("mae") is not None else "-",
                        "R2": round(float(metrics["r2"]), 3)
                        if metrics.get("r2") is not None else "-",
                        "Baseline R2": round(float(baseline["r2"]), 3)
                        if baseline.get("r2") is not None else "-",
                    }
                )
            ...
            st.caption(
                "Metrics are from the held-out test period. "
                "The forecast uses the locally registered Ridge v6 models."
            )
        else:
            st.info("Forecast model metadata is unavailable.")
...
def explain(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        logger.exception("Forecast explanation failed")
